...
__constant__ precision V_d, lag_d, h_d,dtr,small = 1e-20, alpha= 0.001665973;
__device__ precision Energy_d[PARTICLES], v0byc_d[PARTICLES], p0_d[PARTICLES];


  __device__ void
KQ_MAP(int j,int nslice, precision &x1,  precision &px1,   precision &y1,   precision &py1, precision &ct1, precision de1, precision Energyd,precision p0d)
{  
  
  precision e,x,y, px, py,ct,kx,ds,gamma_s,ps,delta_s, beta_s;
  // printf("in KQ_MAP x1=%f mlt=%f %f %f %f el=%i \n",x1,rhic_d[j].mlt[0],rhic_d[j].mlt[1],rhic_d[j].mlt[2],rhic_d[j].mlt[3],j);
  ds = rhic_d[j].length/nslice;
  kx = (rhic_d[j].k1l/rhic_d[j].length);
  // printf("m0_d = %f \n",m0_d);
    e = de1*p0d + Energyd;
    ps = sqrt(e*e - m0_d*m0_d);
   gamma_s = e/m0_d;
   ...
   if(kx>=0) {
     ...
     //  printf("before ds=%f kx=%f length=%f nslice=%i x=%f px=%f y=%f py=%f ct=%f e=%f ps=%f \n",ds,kx,rhic_d[j].length,nslice,x1,px1,y1,py1,ct1,e,ps);
     precision k = sqrt(kx);
  x = cos(k*ds)*x1 + sin(k*ds)*px1/k;
  //   if(i==1)
  // printf("x =%f x1=%f \n",x,x1);
  px = -k*sin(k*ds)*x1 + cos(k*ds)*px1;
  y = cosh(k*ds)*y1 + sinh(k*ds)*py1/k;
  py = k*sinh(k*ds)*y1 + cosh(k*ds)*py1;
   }else {
     precision k = sqrt(-1*kx);
 x = cosh(k*ds)*x1 + sinh(k*ds)*px1/k;
  px = k*sinh(k*ds)*x1 + cosh(k*ds)*px1;
  y = cos(k*ds)*y1 + sin(k*ds)*py1/k;
  py = -k*sin(k*ds)*y1 + cos(k*ds)*py1;
   }
  ct = (alpha*delta_s/beta_s)*ds + ct1 + ds*de1/(beta_s*beta_s*gamma_s*gamma_s);

  x1=x;px1=px;y1=y;py1=py;ct1=ct;
  
  //  printf("ds=%f kx=%f length=%f nslice=%i x=%f px=%f y=%f py=%f ct=%f \n",ds,kx,rhic_d[j].length,nslice,x1,px1,y1,py1,ct1);
}

  __device__ void
KNL_MAP(int j, int nslice, precision &x1,  precision &px1,   precision &y1,   precision &py1, precision &ct1, precision de1, precision Energyd,precision p0d)
{  
  precision x,y,e,ct,p2,pnl,gamma_s,beta_s,ds;
  e = de1*p0d + Energyd;
  gamma_s = e/m0_d;
  beta_s = sqrt(1 - 1/(gamma_s*gamma_s));
  p2 = sqrt(1+2*de1/beta_s + de1*de1 - px1*px1 -py1*py1);
  pnl = 1/p2 -1;
  ds = rhic_d[j].length/nslice;
  x = x1 + pnl*px1*ds;
  y = y1 + pnl*py1*ds;
  ct = ct1 + (1/beta_s - de1/(beta_s*beta_s*gamma_s*gamma_s) -(1/beta_s + de1)/p2)*ds; 
  ...
  __device__ void
QuadProp(int j,int nslice, precision &x1,  precision &px1,   precision &y1,   precision &py1, precision &ct1, precision de1, precision Energyd, precision p0d)
{
  // printf("in Quadrop x1=%f y1=%f \n",x1,y1);
  int ns = nslice*2;
  KQ_MAP(j,ns,x1,px1,y1,py1,ct1,de1,Energyd,p0d);
  KNL_MAP(j,nslice,x1,px1,y1,py1,ct1,de1,Energyd,p0d);
  KQ_MAP(j,ns,x1,px1,y1,py1,ct1,de1,Energyd,p0d);
...
for(int j = 0 ; j < Nelement ; j++)
   {
	 ...
	 if(ang > small){
	   ...
	   BendProp(i,j,x,px,y,py,ct,de,v0byc,p0d,Energyd);
          ...
          if(rhic_d[j].length < 1.0e-3 || rhic_d[j].k1l == 0 ){
	   MultProp(i,j,x,px,y,py,ct,de,v0byc,p0d,Energyd);
	  }else{
          QuadProp(j,2,x,px,y,py,ct,de,Energyd,p0d);
	  }
	 }else if(length > 0) {
...
if(rhic_d[j].length < 1.0e-3 || rhic_d[j].k1l == 0 ){
     MultProp(i,j,x,px,y,py,ct,de,v0byc,p0d,Energyd);
        ...
        int nss = ns*2;
   	QuadProp(j,nss,x,px,y,py,ct,de,Energyd,p0d);
 }
 }  else if( length > small){
