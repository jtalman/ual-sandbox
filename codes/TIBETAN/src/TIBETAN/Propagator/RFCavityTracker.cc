// Library       : TIBETAN
// File          : TIBETAN/Propgator/RFCavityTracker.cc
...
TIBETAN::RFCavityTracker::RFCavityTracker()
{
  init();
}

TIBETAN::RFCavityTracker::RFCavityTracker(const TIBETAN::RFCavityTracker& rft)
{
  copy(rft);
}
...
void TIBETAN::RFCavityTracker::setRF(double V, double h, double lag)
{
  m_V   = V;
  m_h   = h;
  m_lag = lag;
...
UAL::PropagatorNode* TIBETAN::RFCavityTracker::clone()
{
  return new TIBETAN::RFCavityTracker(*this);
}

void TIBETAN::RFCavityTracker::setLatticeElements(const UAL::AcceleratorNode& sequence, 
						  int is0, 
						  int is1,
...
void TIBETAN::RFCavityTracker::setLatticeElement(const PacLattElement& e)
{
  init();
...
void TIBETAN::RFCavityTracker::propagate(UAL::Probe& probe)
{
  ...
  for(int ip = 0; ip < bunch.size(); ip++) {
    ...
    if(m_l) passDrift(m_l/2., p, v0byc_old, vbyc);
    ...
    if(m_l) passDrift(m_l/2., p, v0byc_new, vbyc);
...
void TIBETAN::RFCavityTracker::init()
{
  m_l   = 0.0;
  m_V   = 0.0;
  m_lag = 0.0;
...
void TIBETAN::RFCavityTracker::copy(const TIBETAN::RFCavityTracker& rft)
{
  m_l   = rft.m_l;
  m_V   = rft.m_V;
  m_lag = rft.m_lag;
...
void TIBETAN::RFCavityTracker::passDrift(double l, PAC::Position& p, double v0byc, double vbyc)
{
...
TIBETAN::RfCavityTrackerRegister::RfCavityTrackerRegister()
{
  UAL::PropagatorNodePtr nodePtr(new TIBETAN::RFCavityTracker());
  UAL::PropagatorFactory::getInstance().add("TIBETAN::RfCavityTracker", nodePtr);
}

static TIBETAN::RfCavityTrackerRegister theSingleton; 
