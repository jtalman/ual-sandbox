...
__device__ void SnakeProp(int N,int j,   double &sx,   double &sy,   double &sz,double de, double p0d, double Energyd )
{
  double A[3] ;
  double s_mat[9],e,gamma,Ggam ;
  double cs,sn;
  e = de*p0d + Energyd;
  gamma = e/m0_d;
  Ggam = gamma*GG_d;
  ...
  if( rhic_d[j].snake == 1) {
   ...
   cs = 1.00 -cos(snk1_mu_d*dtr) ; sn =  sin(snk1_mu_d*dtr) ;
  ...
  }else if(rhic_d[j].snake == 3) {
    double mu,phirot,therot;
    mu = 90.0*dtr; phirot = (90.0+0.210471462*Ggam)*dtr;
    therot = 0.0;
     double al5,al6,al7,al8;
   
    cs = 1.00 -cos(mu) ; sn =  sin(mu) ;
    al5 = cos(therot);
    al6 = sin(therot);
    al7 = cos(phirot);
    al8 = sin(phirot);
    ...
    A[1] = al6;
    A[2] = al5*al7;
  }else if(rhic_d[j].snake == 4){
    // std::cout << "doing rot7 \n";
  double mu,phirot,therot;
    mu = -90.0*dtr; phirot = (90-0.210471462*Ggam)*dtr;
    therot = 0.0;
    double al5,al6,al7,al8;
...
for(int j = 0 ; j < Nelement ; j++)
   {
	 ...
	 //    t0_d += length/v;
		...
		if(rhic_d[j].snake > 0 ){
		  ...
		  SnakeProp(i,j,sx,sy,sz,de,p0d,Energyd);} else{
