...
protected:
    ...
    link_type get_node() 
      {return (link_type) (::operator new ((size_t) (1*sizeof(slist_node)))); }
...
#ifdef __GNUG__
    void insert(iterator previous, iterator position, const T* first, const T* last){
      while (first != last) {
	insert(previous, position, *first++);
	++previous;
      }
    }
    void insert(iterator previous, iterator position, const_iterator first, const_iterator last){
      while (first != last) {
	insert(previous, position, *first++);
	++previous;
      }
    }
    void insert(iterator previous, iterator position, size_type n, const T& x){
      while (n--) {
...
template <class T> bool& slist<T>::separate_copying(){
  static bool sep_cop = true;
  return sep_cop;
}
