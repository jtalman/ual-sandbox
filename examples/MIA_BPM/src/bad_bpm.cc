...
int main(){
  ...
  for(ibpm = bpms.begin(); ibpm != bpms.end(); ibpm++){
    // std::cout << ibpm->first << std::endl;
  }

  // int bad_bpms[] = {4, 6};
  Index_Noise bad_bpms[] = {{4, 2.e-5}, {6, 3.e-5}};

  for (int ib = 0; ib < 2; ib++) {
    
    int index = bad_bpms[ib].index;
    double error = bad_bpms[ib].error;

    std::cout << ib << " index = " << index << ", error = " << error << std::endl;

    ibpm = bpms.find(index);

    if(ibpm != bpms.end()){ // update turn-by-turn data
      std::list<PAC::Position>::iterator it;
      for(it = ibpm->second->getData().begin(); it != ibpm->second->getData().end(); it++){

	double r = randomGenerator.getNumber(iseed);
	it->setX(error*r);
