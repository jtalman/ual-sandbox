...
// Constructor.
UAL_SXF_ElemRfCavity::UAL_SXF_ElemRfCavity(SXF::OStream& out)
  : UAL_SXF_ElemBucket(out, "body", new SXF::ElemRfCavityHash())
{
  m_iRfStatus = 0;
...
// Destructor.
UAL_SXF_ElemRfCavity::~UAL_SXF_ElemRfCavity()
{
  if(m_pRfCavity) delete m_pRfCavity;
}

// Zero bucket.
void UAL_SXF_ElemRfCavity::close()
{
  m_pRfCavity->order(-1);
  m_pRfCavity->order(0);
  m_iRfStatus = 0;
...
int UAL_SXF_ElemRfCavity::setScalarValue(double v)
{
  int result = SXF_TRUE;
...
int UAL_SXF_ElemRfCavity::getBodySize() const
{
  return 1;
}
...
PacElemBucket* UAL_SXF_ElemRfCavity::getBodyBucket(int index) 
{
  if(index == 0)
...
// Write data
void UAL_SXF_ElemRfCavity::write(ostream& out, const PacLattElement& element, const string& tab)
{
  // Get body attributes
