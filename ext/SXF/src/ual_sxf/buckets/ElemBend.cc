...
int UAL_SXF_ElemBend::s_iMaxOrder = UAL_SXF_MAX_ORDER;

// Constructor.
UAL_SXF_ElemBend::UAL_SXF_ElemBend(SXF::OStream& out)
  : UAL_SXF_ElemBucket(out, "body", new SXF::ElemBendHash())
{
  // Deviation flag
  m_iIsDeviation = 0;
...
// Destructor.
UAL_SXF_ElemBend::~UAL_SXF_ElemBend()
{
  // Bend data
  for(int i=0; i < 3; i++){
...
int UAL_SXF_ElemBend::openObject(const char* type)
{
...
int UAL_SXF_ElemBend::openArray()
{
  int result = SXF_TRUE;
...
// Do nothing.
void UAL_SXF_ElemBend::closeArray()
{
}

// Check the array attribute key (KL or KLS). 
// Return SXF_TRUE or SXF_FALSE.
int UAL_SXF_ElemBend::openHash()
{
  return openArray();
}

// Do nothing. 
void UAL_SXF_ElemBend::closeHash()
{
}

// Check the scalar attribute key (FINT, E1, E2) and set its value. 
// Skip the HGAP attribute.
int UAL_SXF_ElemBend::setScalarValue(double v)
{
  int result = SXF_TRUE;
...
// not a deviation.
int UAL_SXF_ElemBend::setArrayValue(double v)
{
  int result = SXF_TRUE;
...
int UAL_SXF_ElemBend::setHashValue(double v, int index)
{
  int result = SXF_TRUE;
...
int UAL_SXF_ElemBend::getEntrySize() const
{
  return 1;
}
...
int UAL_SXF_ElemBend::getBodySize() const
{
  return 2;
}
...
int UAL_SXF_ElemBend::getExitSize() const
{
  return 1;
}
...
PacElemBucket* UAL_SXF_ElemBend::getEntryBucket(int index) 
{
  if(index == 0) 
...
// Return 0, if the bucket is not defined or empty.
PacElemBucket* UAL_SXF_ElemBend::getExitBucket(int index) 
{
  if(index == 0) 
...
// Return 0, if the bucket is not defined or empty.
PacElemBucket* UAL_SXF_ElemBend::getBodyBucket(int index) 
{
  if(index == 0) { // Bend
...
void UAL_SXF_ElemBend::write(ostream& out, 
...
void UAL_SXF_ElemBend::makeMltFactors()
{ 
