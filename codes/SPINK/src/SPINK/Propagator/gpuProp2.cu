...
__constant__ precision V_d, lag_d, h_d,dtr;
 ...
 det = de;
  ...
  makeRVgpuP(N, de, det);//
    ...
    if(rhic_d[j].m_ir == 0){
	 ...
	 applyMltKickgpuP(N,j,1.0,1,rhic_d[j].dx,rhic_d[j].dy,x, px, y, py); //
      }
      applyThinBendKickgpuP(N,j,1,rhic_d[j].dx,rhic_d[j].dy, x,px,y,py,ct, de);//
      makeVelocitygpuP(N,px, py,de,xt,pxt, pyt);//
   ...
   for(int i = 0; i < rhic_d[j].m_ir; i++){
     for(int is = 1; is < 5; is++){
       ...
       if(rhic_d[j].mlt[0] != 1000){
	 applyMltKickgpuP(N,j,rkicks,1,rhic_d[j].dx,rhic_d[j].dy,x, px, y, py); //
 }
       applyThinBendKickgpuP(N,j,rkicks,rhic_d[j].dx,rhic_d[j].dy,x, px, y, py,ct,de);//
	makeVelocitygpuP(N,px, py,de,xt,pxt, pyt);
     }
     counter++;
     passBendSlicegpuP(N,j,counter, x, px, y, py, ct,de, xt, pxt, yt, pyt, det);
...
__device__ void MultProp(int N, int j, volatile precision &x, volatile precision &px, volatile precision &y, volatile precision &py, volatile precision &ct, volatile precision &de)
{
 ...
 det = de;
  ...
  makeRVgpuP(N, de, det);//
   ...
   if(rhic_d[j].mlt[0] != 1000) {
   applyMltKickgpuP(N,j,1.,1,rhic_d[j].dx,rhic_d[j].dy,x, px, y, py);//
    }
   makeVelocitygpuP(N,px, py,de,xt,pxt, pyt);
   passDriftgpuP(N,rhic_d[j].m_l/2.00, x, pxt, y, pyt, ct, det);
   if(rhic_d[j].exitMlt[0] != 1000) {
