...
SXF_TRACKER::rfCavity::rfCavity()
  : TEAPOT::BasicTracker()
{
  init();
}

SXF_TRACKER::rfCavity::rfCavity(const SXF_TRACKER::rfCavity& rft)
...
void SXF_TRACKER::rfCavity::setRF(double V, double h, double lag)
{
  m_V   = V;
  m_h   = h;
  m_lag = lag;
...
UAL::PropagatorNode* SXF_TRACKER::rfCavity::clone()
{
  return new SXF_TRACKER::rfCavity(*this);
}
...
void SXF_TRACKER::rfCavity::setLatticeElement(const PacLattElement& e)
{
  init();
...
void SXF_TRACKER::rfCavity::propagate(UAL::Probe& probe)
{
  //   std::cout << "SXF_TRACKER::rfCavity " << m_name << std::endl;
...
void SXF_TRACKER::rfCavity::init()
{
  m_l   = 0.0;
  m_V   = 0.0;
  m_lag = 0.0;
...
void SXF_TRACKER::rfCavity::copy(const SXF_TRACKER::rfCavity& rft)
{
  m_l   = rft.m_l;
  m_V   = rft.m_V;
  m_lag = rft.m_lag;
...
void SXF_TRACKER::rfCavity::passDrift(double l, PAC::Position& p, double v0byc, double vbyc)
{
...
SXF_TRACKER::rfCavityRegister::rfCavityRegister()
{
  UAL::PropagatorNodePtr rfPtr(new SXF_TRACKER::rfCavity());
  UAL::PropagatorFactory::getInstance().add("SXF_TRACKER::rfCavity", rfPtr);
}
