...
// Library       : SPINK
// File          : SPINK/Propagator/SnakeTransform.cc
// Copyright     : see Copyright file
// Author        : A.Luccio
// C++ version   : N.Malitsky, V.Ptitsyn, N.D'Imperio
// revised       : A.U.Luccio 12DEC09
...
double SPINK::SnakeTransform::snk1_mu = 0.0;
double SPINK::SnakeTransform::snk2_mu = 0.0;
double SPINK::SnakeTransform::snk1_phi = 0.0;
double SPINK::SnakeTransform::snk2_phi = 0.0;
double SPINK::SnakeTransform::snk1_theta = 0.0;
double SPINK::SnakeTransform::snk2_theta = 0.0;

/** pass variables for diagnostics AUL:02MAR10 */
bool SPINK::SnakeTransform::coutdmp = 0;
int SPINK::SnakeTransform::nturn = 0;

SPINK::SnakeTransform::SnakeTransform()
{
  p_length = 0;
}

SPINK::SnakeTransform::SnakeTransform(const SPINK::SnakeTransform& st)
{
  copy(st);
}
...
UAL::PropagatorNode* SPINK::SnakeTransform::clone()
{
  return new SPINK::SnakeTransform(*this);
}


void SPINK::SnakeTransform::setLatticeElements(const UAL::AcceleratorNode& sequence,
					   int is0, int is1,
...
void SPINK::SnakeTransform::propagate(UAL::Probe& b)
{
...
void SPINK::SnakeTransform::setElementData(const PacLattElement& e)
{
...
void SPINK::SnakeTransform::setConventionalTracker(const UAL::AcceleratorNode& sequence,
                                                int is0, int is1,
                                                const UAL::AttributeSet& attSet)
{
    const PacLattice& lattice = (PacLattice&) sequence;
...
void SPINK::SnakeTransform::propagateSpin(UAL::Probe& b)
{
    PAC::Bunch& bunch = static_cast<PAC::Bunch&>(b);
...
/*
void SPINK::SnakeTransform::setSnakeParams(mu1, mu2, phi1, phi2, the1, the2) //AUL:10FEB10
{
...
void SPINK::SnakeTransform::propagateSpin(PAC::BeamAttributes& ba, PAC::Particle& prt)
{
  ...
  if( m_name == "snake1") {
    ...
    double cs = 1. -cos(snk1_mu*dtr) ; double sn =  sin(snk1_mu*dtr) ;
    ...
    if( coutdmp ){ //AUL:01MAR10
      std::cout << "\nSnakeTransform " << m_name << ", turn = " << nturn << endl ;
    ...
    double cs = 1. -cos(snk2_mu*dtr) ; double sn =  sin(snk2_mu*dtr) ;
...
void SPINK::SnakeTransform::copy(const SPINK::SnakeTransform& st)
{
    m_name       = st.m_name;
...
SPINK::SnakeTransformRegister::SnakeTransformRegister()
{
  UAL::PropagatorNodePtr snakePtr(new SPINK::SnakeTransform());
  UAL::PropagatorFactory::getInstance().add("SPINK::SnakeTransform", snakePtr);
}

static SPINK::SnakeTransformRegister theSpinkSnakeTransformRegister;
