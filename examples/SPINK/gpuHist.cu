...
int main(){
  ...
  std::ifstream configInput("./datagpu/spinkISF.in");//AULNLD:07JAN10
  ...
  configInput >> dummy >> SpinAlignFile;
  double xbin[20][20],ybin[20][20],Sxbin[20][20],Sybin[20][20],Szbin[20][20];
  if(SpinAlignFile == 1){
  
  std::ifstream SpinIN("histogram.dat");
      for(int ibx=0; ibx< 20; ibx++){
	for(int iby=0; iby < 20; iby++){
	SpinIN >> xbin[ibx][iby] >> ybin[ibx][iby] >> Sxbin[ibx][iby] >>Sybin[ibx][iby] >> Szbin[ibx][iby] ;}
      }}
  ...
  */
  ...
  PAC::Position closed_orbit;
  optics.m_teapot->clorbit(closed_orbit,ba);
  double xco = closed_orbit.getX();
  double pxco = closed_orbit.getPX();
  double yco = closed_orbit.getY();
  double pyco = closed_orbit.getPY();
...
do {
  ...
  sigct = sigt*cc;
    x0  = rngx *sigx + xco;
    x0p = rngxp*sigxp + pxco;
    y0  = rngy *sigy+ yco;
    y0p = rngyp*sigyp+ pyco;
 ...
 if(SpinAlignFile == 1){
   for(int ibx = 0 ; ibx < 9; ibx++){
     for(int iby = 0; iby < 9; iby++){
       if(y0>ybin[ibx][iby] && y0 < ybin[ibx][iby+1] && 
	  y0p>xbin[ibx][iby] && y0p < xbin[ibx+1][iby]){
	 ssx = Sxbin[ibx][iby]; ssy = Sybin[ibx][iby]; ssz = Szbin[ibx][iby];}
     }}
   std::cout << "Spin is =" << ssx << " " << ssy << " " << ssz  << " ";
spin.setSX(ssx);
  spin.setSY(ssy);
  spin.setSZ(ssz);}
...
int bins = 20;
  ...
  double ymin = -4.5e-4;
  double ymax = 4.5e-4;
 gsl_histogram2d * hy = gsl_histogram2d_alloc(bins,bins);
 gsl_histogram2d * hx = gsl_histogram2d_alloc(bins,bins);
 gsl_histogram2d * hz = gsl_histogram2d_alloc(bins,bins);
 gsl_histogram2d * h = gsl_histogram2d_alloc(bins,bins);
  
  if(SpinAlignFile == 3){
  
 gsl_histogram2d_set_ranges_uniform(hy,
				   xmin,xmax,
				   ymin,ymax);
 gsl_histogram2d_set_ranges_uniform(hx,
				   xmin,xmax,
				   ymin,ymax);
 gsl_histogram2d_set_ranges_uniform(hz,
				   xmin,xmax,
				   ymin,ymax);
...
gsl_histogram2d_accumulate (hy,pos[ip].y,pos[ip].py,pos[ip].sy);
gsl_histogram2d_accumulate (hx,pos[ip].y,pos[ip].py,pos[ip].sx);
gsl_histogram2d_accumulate (hz,pos[ip].y,pos[ip].py,pos[ip].sz);
	 gsl_histogram2d_accumulate (h,pos[ip].y,pos[ip].py,1);
     }
	 }
...
for(int ip = 0; ip < N; ip++) {
  ...
  // }
   ...
   if(SpinAlignFile == 3){
     std::ofstream hist;
     hist.open("histogram.dat");
   double xstep = (xmax-xmin)/bins;
   double ystep = (ymax-ymin)/bins;
   double T, xb,yb, SAx,SAz,SAy,Sy,Sx,Sz;
   std::cout << "xstep = " << xstep << "ystep = " << ystep << " \n";
   std::cout << " xmax = " << xmax << "xmin= " << xmin  << "bins = " << bins << " \n";
   for( int ix = 0; ix < bins; ix++){
     for(int iy = 0; iy < bins; iy++){
       T = gsl_histogram2d_get(h,ix,iy);
       Sy = gsl_histogram2d_get(hy,ix,iy);
       Sx = gsl_histogram2d_get(hx,ix,iy);
       Sz = gsl_histogram2d_get(hz,ix,iy);
       if(T > 0){SAx = Sx/T; SAy = Sy/T; SAz = Sz/T;
        
       }else{ SAx = SAy= SAz = 0.0;}
       
       xb = xmin + xstep*ix;
       yb = ymin + ystep*iy;
       sprintf(line, " %e %e %e %e %e \n",xb,yb,SAx,SAy,SAz);
       hist << line;
     }
     hist << " \n";
   ...
   gsl_histogram2d_free(hy);
   gsl_histogram2d_free(hx);
   gsl_histogram2d_free(hz);
   }
