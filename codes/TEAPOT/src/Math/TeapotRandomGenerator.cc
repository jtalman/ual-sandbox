// Library     : Teapot
// File        : Math/TeapotRandomGenerator.cc
...
// Constructor
TeapotRandomGenerator::TeapotRandomGenerator(int seed)
{
  // FTPOT parameters
...
// Destructor
TeapotRandomGenerator::~TeapotRandomGenerator()
{
  if(irn_) delete [] irn_;
}
...
// FTPOT: getran(cut)
double TeapotRandomGenerator::getran(int cut)
{
  if(cut == 0) cut = 2;
...
// Set seed
void TeapotRandomGenerator::setSeed(int seed)
{
  init55(seed);
} 

// Get seed
int TeapotRandomGenerator::getSeed() const
{
  return seed_;
}
...
void TeapotRandomGenerator::init55(int seed)
{
  seed_ = seed;

  int lmod1 = seed_/TEAPOT_RANDOM_MAXINT;
  irn_[nr_-1] = seed_ - lmod1*TEAPOT_RANDOM_MAXINT;
...
// The values generated are integers in the range [0, TEAPOT_RANDOM_MAXINT).
...
int TeapotRandomGenerator::irngen()
{
  int nrmj = nr_ - nj_;
...
double TeapotRandomGenerator::fran()
{
...
void TeapotRandomGenerator::ndrn(double& r_gr1, double& r_gr2)
{
  double gr1, gr2, zzr;
  while(1){
